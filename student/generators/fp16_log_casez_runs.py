#!/usr/bin/env python3
"""casez cube-cover candidates for FP16 log/log2 mantissa runs."""
...
def interval_cubes(lo, hi):
    cubes = []
    while lo <= hi:
        size = 1
        while True:
            next_size = size << 1
            if lo % next_size == 0 and lo + next_size - 1 <= hi:
                size = next_size
            else:
                break
        while lo + size - 1 > hi:
            size >>= 1
        mask = (~(size - 1)) & ((1 << MANT_BITS) - 1)
        cubes.append((lo, mask))
        lo += size
    return cubes


def casez_pattern(value, mask):
    chars = []
    for bit in range(MANT_BITS - 1, -1, -1):
        if (mask >> bit) & 1:
            chars.append("1" if ((value >> bit) & 1) else "0")
        else:
            chars.append("?")
    return "{0}'b{1}".format(MANT_BITS, "".join(chars))


def render_casez_runs(outputs, module, threshold):
    ...
    for exp in range(1, EXP_MAX):
        ...
        for lo, hi, value in runs:
            for cube_value, cube_mask in interval_cubes(lo, hi):
                cubes.append((cube_value, cube_mask, value))
        lines.append("        {0}: begin".format(const_word(EXP_BITS, exp)))
        if len(cubes) <= threshold:
            cube_groups += 1
            cube_items += len(cubes)
            lines.append("          casez (mant)")
            for cube_value, cube_mask, value in cubes:
                lines.append("            {0}: out_r = {1};".format(casez_pattern(cube_value, cube_mask), const_word(16, value)))
    ...
    lines.extend(footer())
    meta = "threshold={0}; cube_groups={1}; table_groups={2}; cube_items={3}; table_items={4}".format(
        threshold,
        cube_groups,
        table_groups,
        cube_items,
...
def run_case(args, case, refs):
    ...
    for threshold in THRESHOLDS:
        variant = "casez_runs_t{0}".format(threshold)
        ...
        start = time.time()
        try:
            verilog, items, model = render_casez_runs(outputs, module, threshold)
            row["items"] = items
            ...
            if verified:
                ...
                evaluated = evaluate_aig(
                    ...
                    parent_id=label,
                    source="ai_verilog",
                    tool_chain="yosys_fp16_log_casez_runs_t{0}".format(threshold),
                    aig_path=aig_path,
                    truth=truth_path,
                    abc=args.abc,
                    notes="casez run-cube FP16 log candidate; {0}".format(row["items"]),
                    timeout=args.timeout,
                )
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="casez cube-cover FP16 log/log2 candidates.")
