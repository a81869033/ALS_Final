module ex245_r11_r89_active_branch_muxclean(in, out);
  input [15:0] in;
  output [7:0] out;
  wire [7:0] a = in[15:8];
  wire [7:0] b = in[7:0];
  ...
  wire [6:0] same_mag_tbl = same_class_mag({gap[1:0], max_sig, min_sig}, min_eff);
  wire [6:0] diff_mag_tbl = diff_class_mag({gap[2:0], max_sig, min_sig}, min_eff);
  wire diff_sign = (maxc == minc) ? 1'b0 : max_sign;
  wire [7:0] same_out = {a[7], same_mag_tbl};
  wire [7:0] diff_out = {diff_sign, diff_mag_tbl};
  wire [7:0] pass_out = {(same_sign ? a[7] : diff_sign), maxc};
  wire [7:0] active_out = use_same_table ? same_out : diff_out;
  wire [7:0] selected_out = (use_same_table || use_diff_table) ? active_out : pass_out;
  assign out = nan ? 8'h7f : selected_out;
endmodule
