module ex248_r8_pruned_periodic_core_abc_g_aig_frontend(in, out);
  input [15:0] in;
  output [7:0] out;
  ...
  wire num_sign = in[7];
  wire [7:0] core_out;
  ex248_r8_pruned_periodic_core core(.in(in), .out(core_out));
  wire nan = (den == 7'h7f) || (num == 7'h7f);
  wire den_zero = (den == 7'h00);
  wire num_zero = (num == 7'h00);
  wire num_lt_den = (num < den);
  wire num_eq_den = (num == den);
  assign out = (nan || den_zero) ? 8'h7f :
               num_zero ? {num_sign, 7'h00} :
               num_lt_den ? {num_sign, num} :
               num_eq_den ? {num_sign, 7'h00} :
               core_out;
endmodule

module ex248_r8_pruned_periodic_core(in, out);
  input [15:0] in;
  output [7:0] out;
  wire [6:0] den = in[14:8];
  wire [6:0] num = in[6:0];
