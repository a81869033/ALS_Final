module ex248_r11_exp_sig_compare(in, out);
  input [15:0] in;
  output [7:0] out;
  ...
  wire num_sign = in[7];
  wire [6:0] core_mag;
  ex248_r11_exp_sig_compare_core core(.in(in), .out(core_mag));
  wire nan = (den == 7'h7f) || (num == 7'h7f);
  wire den_zero = (den == 7'h00);
  wire num_zero = (num == 7'h00);
  wire num_exp_lt_den = (num[6:2] < den[6:2]);
  wire num_exp_eq_den = (num[6:2] == den[6:2]);
  wire num_sig_lt_den = (num[1:0] < den[1:0]);
  wire num_lt_den = num_exp_lt_den || (num_exp_eq_den && num_sig_lt_den);
  wire num_eq_den = (num == den);
  assign out[7] = (nan || den_zero) ? 1'b0 : num_sign;
  assign out[6:0] = (nan || den_zero) ? 7'h7f :
                    (num_zero ? 7'h00 :
                    (num_lt_den ? num :
                    (num_eq_den ? 7'h00 : core_mag)));
endmodule

module ex248_r11_exp_sig_compare_core(in, out);
  input [15:0] in;
  output [6:0] out;
  wire [6:0] den = in[14:8];
  wire [6:0] num = in[6:0];
