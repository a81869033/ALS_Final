module ex248_r10_output_plane_split(in, out);
  input [15:0] in;
  output [7:0] out;
  wire [7:0] core_out;
  ex248_r10_output_plane_split_core core(.in(in), .out(core_out));
...
module ex248_r10_output_plane_split_core(in, out);
  input [15:0] in;
  output [7:0] out;
  ...
  wire num_sign = in[7];
  wire [7:0] core_out;
  ex248_r8_split_payload_core core(.in(in), .out(core_out));
  wire nan = (den == 7'h7f) || (num == 7'h7f);
  wire den_zero = (den == 7'h00);
  wire num_zero = (num == 7'h00);
  wire num_lt_den = (num < den);
  wire num_eq_den = (num == den);
  assign out[7] = (nan || den_zero) ? 1'b0 : num_sign;
  assign out[6:0] = (nan || den_zero) ? 7'h7f :
                    (num_zero ? 7'h00 :
                    (num_lt_den ? num :
                    (num_eq_den ? 7'h00 : core_out[6:0])));
endmodule

module ex248_r8_split_payload_core(in, out);
  input [15:0] in;
  output [7:0] out;
  wire [6:0] den = in[14:8];
  wire [6:0] num = in[6:0];
