module ex248_r11_action_mag_select(in, out);
  input [15:0] in;
  output [7:0] out;
  ...
  wire num_sign = in[7];
  wire [6:0] core_mag;
  ex248_r11_action_mag_select_core core(.in(in), .out(core_mag));
  wire nan = (den == 7'h7f) || (num == 7'h7f);
  wire den_zero = (den == 7'h00);
  wire num_zero = (num == 7'h00);
  wire num_lt_den = (num < den);
  wire num_eq_den = (num == den);
  wire invalid = nan || den_zero;
  reg [6:0] mag_sel;
  always @* begin
    casez ({num_zero, num_lt_den, num_eq_den})
      3'b1??: mag_sel = 7'h00;
      3'b01?: mag_sel = num;
      3'b001: mag_sel = 7'h00;
      default: mag_sel = core_mag;
    endcase
  end
  assign out = invalid ? 8'h7f : {num_sign, mag_sel};
endmodule

module ex248_r11_action_mag_select_core(in, out);
  input [15:0] in;
  output [6:0] out;
  wire [6:0] den = in[14:8];
  wire [6:0] num = in[6:0];
