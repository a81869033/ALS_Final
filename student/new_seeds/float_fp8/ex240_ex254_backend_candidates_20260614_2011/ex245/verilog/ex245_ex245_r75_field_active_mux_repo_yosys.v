module ex245_r75_field_active_mux(in, out);
  input [15:0] in;
  output [7:0] out;
  wire [7:0] a = in[15:8];
  wire [7:0] b = in[7:0];
  ...
  wire a_ge_b = (am[6:2] > bm[6:2]) || ((am[6:2] == bm[6:2]) && (am[1:0] >= bm[1:0]));
  ...
  wire [5:0] a_eff = (am[6:2] == 5'd0) ? 6'd1 : {1'b0, am[6:2]};
  wire [5:0] b_eff = (bm[6:2] == 5'd0) ? 6'd1 : {1'b0, bm[6:2]};
  wire [2:0] a_sig = (am[6:2] == 5'd0) ? {1'b0, am[1:0]} : {1'b1, am[1:0]};
  wire [2:0] b_sig = (bm[6:2] == 5'd0) ? {1'b0, bm[1:0]} : {1'b1, bm[1:0]};
  wire [2:0] max_sig = a_ge_b ? a_sig : b_sig;
  wire [2:0] min_sig = a_ge_b ? b_sig : a_sig;
  wire [5:0] max_eff = a_ge_b ? a_eff : b_eff;
  wire [5:0] min_eff = a_ge_b ? b_eff : a_eff;
  wire [5:0] gap = max_eff - min_eff;
  wire use_same_table = same_sign && (gap <= 6'd3);
  wire use_diff_table = (!same_sign) && (gap <= 6'd4);
  wire [6:0] same_mag = same_class_mag({gap[1:0], max_sig, min_sig}, min_eff);
  wire [6:0] diff_mag = diff_class_mag({gap[2:0], max_sig, min_sig}, min_eff);
  wire [6:0] out_mag = use_same_table ? same_mag : (use_diff_table ? diff_mag : maxc);
