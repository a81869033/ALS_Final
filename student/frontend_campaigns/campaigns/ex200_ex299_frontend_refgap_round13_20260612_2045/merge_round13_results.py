#!/usr/bin/env python3
"""Merge round13 results using the round8 coordinator merge implementation."""
...
RUN_ID = "ex200_ex299_frontend_refgap_round13_20260612_2045"

namespace = {"__name__": "round13_merge_from_round8", "__file__": str(TEMPLATE)}
...
def update_search_state_round13(changed):
    ...
    for case, old, new, cand in changed:
        ...
        if not row:
            continue
        row["status"] = "round13_evaluated_gap_remains"
        row["assigned_agent"] = "round13"
        row["current_focus"] = (
            "round13 official frontend improvement accepted: "
            "{0} ADP {1}->{2}".format(cand, old, new)
        )
        row["next_hypothesis"] = "continue only with materially different sharing; reference gap remains"
        row["next_representation"] = "lower-delay/lower-area high-level source, not same round13 replay"
        row["budget"] = "continue"
        row["blocked_reason"] = ""
        row["last_update"] = "2026-06-12 round13 coordinator merge"
...
def update_hypothesis_bank_round13(changed):
    if not changed:
        return
    ...
    for case, old, new, cand in changed:
        rows.append(
            {
                "case": case,
                "hypothesis": "round13 accepted frontend improvement",
                "representation_family": cand,
                "sharing_strategy": "official evaluate.py candidate from round13 shard",
                "source": RUN_ID,
                "confidence": "high",
                "novelty": "exploit",
...
namespace["read_rows"] = read_rows_round13
namespace["update_search_state"] = update_search_state_round13
namespace["update_hypothesis_bank"] = update_hypothesis_bank_round13
namespace["main"]()
