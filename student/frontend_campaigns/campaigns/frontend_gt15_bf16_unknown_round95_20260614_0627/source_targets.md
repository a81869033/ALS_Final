...
Do not use `current_frontend_gap.csv` backend best rows as frontend seeds.
...
Round95 emphasis:
- `ex205`, `ex214`: small threshold gaps. Prefer low-risk recuts that reduce
  one level or a small amount of area.
- `ex216`, `ex218`, `ex219`: current semantic families need new field-level
  grouping, not more rejected broad hi/lo or default-expression sweeps.
- `ex286`, `ex287`: selected-output direct ROBDD hybrids and symcof/casemux
  replays are exhausted. Try different high-level routing/state bases or prove
  blockers concretely.
