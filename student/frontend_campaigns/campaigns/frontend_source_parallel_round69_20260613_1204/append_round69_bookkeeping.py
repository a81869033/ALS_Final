...
RUN_ID = "frontend_source_parallel_round69_20260613_1204"
...
def main():
    ...
    for row in candidates:
        ...
        source = "{}/{}".format(RUN_ID, row.get("agent_id", "coordinator-round69"))
        key = (row.get("case", ""), hypothesis, representation, source)
        if key in existing_hyp:
            continue
        hyp_add.append(
            {
                "case": row.get("case", ""),
                "hypothesis": compact(hypothesis, 160),
                "representation_family": compact(representation, 120),
                ...
                "created_by": row.get("agent_id") or "coordinator-round69",
                "created_at": DATE,
            }
        )
