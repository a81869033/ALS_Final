#!/usr/bin/env python3
"""Merge round16 frontend campaign shard CSVs."""
...
RUN_ID = "ex200_ex299_frontend_refgap_round16_20260612_2233"

namespace = {"__name__": "round16_merge_from_round8", "__file__": str(TEMPLATE)}
...
def read_rows_round16(path):
    ...
    if Path(path).name == "candidates.csv":
        ...
        official_by_id = {
            ...
            if row.get("official_evaluate_status", row.get("official_status", "")) in ("OK", "")
        }
        kept = []
        for row in rows:
            if row.get("equivalent") != "1":
                continue
            status = row.get("official_evaluate_status") or row.get("official_status")
...
def update_search_state_round16(changed):
    ...
    for case, old, new, cand in changed:
        ...
        if not row:
            continue
        row["status"] = "round16_evaluated_gap_remains"
        row["assigned_agent"] = "round16"
        row["current_focus"] = (
            "round16 official frontend improvement accepted: "
            "{0} ADP {1}->{2}".format(cand, old, new)
        )
        row["next_hypothesis"] = "continue only with materially different sharing; reference gap remains"
        row["next_representation"] = "lower-delay/lower-area high-level source, not same round16 replay"
        row["budget"] = "continue"
        row["blocked_reason"] = ""
        row["last_update"] = "2026-06-12 round16 coordinator merge"
...
def update_hypothesis_bank_round16(changed):
    if not changed:
        return
    ...
    for case, old, new, cand in changed:
        rows.append(
            {
                "case": case,
                "hypothesis": "round16 accepted frontend improvement",
                "representation_family": cand,
                "sharing_strategy": "official evaluate.py candidate from round16 shard",
                "source": RUN_ID,
                "confidence": "high",
                "novelty": "exploit",
...
namespace["read_rows"] = read_rows_round16
namespace["update_search_state"] = update_search_state_round16
namespace["update_hypothesis_bank"] = update_hypothesis_bank_round16
namespace["main"]()
