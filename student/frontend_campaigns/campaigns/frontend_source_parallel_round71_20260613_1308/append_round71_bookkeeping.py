...
DATE = "2026-06-13"
BEST_IDS = {"ex242_r71_axis_attr_hardrom"}
...
def is_best(row):
    return row.get("candidate_id") in BEST_IDS
...
def main():
            ...
            manual_add.append(
                {
                    ...
                    "failure_reason": "official evaluate.py equivalent and improved current ADP",
                    "reuse_policy": "build_on_with_material_change",
                    "next_action": "Explore smaller hard-zone ROM and output-group hybrids around this source family.",
                    "author": "coordinator-round71",
                    "date": DATE,
    ...
    for row in candidates:
        ...
        source = "{}/{}".format(RUN_ID, row.get("agent_id", "coordinator-round71"))
        ...
        hyp_add.append(
            {
                ...
                "reason": "official OK improved current ADP to {}".format(row.get("adp", ""))
                if best
                else "official OK ADP {} did not beat current frontend/reference".format(row.get("adp", "")),
                "created_by": row.get("agent_id") or "coordinator-round71",
                "created_at": DATE,
            }
        )
