#!/usr/bin/env python3
"""Merge round11 results using the round8 coordinator merge implementation."""
...
RUN_ID = "ex200_ex299_frontend_refgap_round11_20260612_1944"

namespace = {"__name__": "round11_merge_from_round8", "__file__": str(TEMPLATE)}
...
def update_search_state_round11(changed):
    ...
    for case, old, new, cand in changed:
        ...
        if not row:
            continue
        row["status"] = "round11_evaluated_gap_remains"
        row["assigned_agent"] = "round11"
        row["current_focus"] = (
            "round11 official frontend improvement accepted: "
            "{0} ADP {1}->{2}".format(cand, old, new)
        )
        row["next_hypothesis"] = "continue only with materially different sharing; reference gap remains"
        row["next_representation"] = "lower-delay/lower-area high-level source, not same round11 replay"
        row["budget"] = "continue"
        row["blocked_reason"] = ""
        row["last_update"] = "2026-06-12 round11 coordinator merge"
...
def update_hypothesis_bank_round11(changed):
    if not changed:
        return
    ...
    for case, old, new, cand in changed:
        rows.append(
            {
                "case": case,
                "hypothesis": "round11 accepted frontend improvement",
                "representation_family": cand,
                "sharing_strategy": "official evaluate.py candidate from round11 shard",
                "source": RUN_ID,
                "confidence": "high",
                "novelty": "exploit",
...
namespace["read_rows"] = read_rows_round11
namespace["update_search_state"] = update_search_state_round11
namespace["update_hypothesis_bank"] = update_hypothesis_bank_round11
namespace["main"]()
