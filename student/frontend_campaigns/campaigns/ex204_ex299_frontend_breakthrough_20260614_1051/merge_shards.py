...
RUN_ID = "ex204_ex299_frontend_breakthrough_20260614_1051"
ROOT = Path(__file__).resolve().parents[4]
BASE = ROOT / "student" / "frontend_campaigns" / "campaigns" / RUN_ID
RESULTS = BASE / "results"
AGENTS = [
    "unknown-breakthrough-a",
    "unknown-breakthrough-b",
    "fp8-breakthrough",
    "fp8-breakthrough-r2",
    "ex247-breakthrough-r3",
    "ex247-breakthrough-r4",
    "ex247-breakthrough-r5",
    "ex247-breakthrough-r6",
    "fp8-tail-transfer-r1",
    "fp8-source-r7",
    "integer-square-r5",
    "unknown-hard-r6",
    "unknown-global-r6",
...
def normalize(row, agent, fname, source_csv):
    ...
    candidate = first(row, ["candidate", "candidate_id", "new_candidate_id", "name"])
    ...
    if fname == "best_improvements.csv":
        ...
        if not row.get("new_adp"):
            row["new_adp"] = first(row, ["adp", "ADP", "best_adp"])
        ...
        if not row.get("adp"):
            row["adp"] = first(row, ["new_adp", "ADP", "best_adp"])
        if not row.get("old_adp"):
            row["old_adp"] = first(row, [
...
def make_case_summary():
    baseline = {}
    for row in read_rows(RESULTS / "target_frontend_snapshot.csv"):
