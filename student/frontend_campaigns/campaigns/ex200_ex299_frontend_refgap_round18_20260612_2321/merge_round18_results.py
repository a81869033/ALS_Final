#!/usr/bin/env python3
"""Merge round18 frontend campaign shard CSVs."""
...
RUN_ID = "ex200_ex299_frontend_refgap_round18_20260612_2321"

namespace = {"__name__": "round18_merge_from_round8", "__file__": str(TEMPLATE)}
    ...
    if Path(path).name == "candidates.csv":
        ...
        for row in rows:
            ...
            if status and status != "OK":
                continue
            if namespace["as_int"](row.get("adp")) is None:
                continue
...
def update_search_state_round18(changed):
    ...
    for case, old, new, cand in changed:
        ...
        if not row:
            continue
        row["status"] = "round18_evaluated_gap_remains"
        row["assigned_agent"] = "round18"
        row["current_focus"] = (
            "round18 official frontend improvement accepted: "
            "{0} ADP {1}->{2}".format(cand, old, new)
        )
        row["next_hypothesis"] = "continue only with materially different sharing; reference gap remains"
        row["next_representation"] = "lower-delay/lower-area high-level source, not same round18 replay"
        row["budget"] = "continue"
        row["blocked_reason"] = ""
        row["last_update"] = "2026-06-12 round18 coordinator merge"
...
def update_hypothesis_bank_round18(changed):
    if not changed:
        return
    ...
    for case, old, new, cand in changed:
        rows.append(
            {
                "case": case,
                "hypothesis": "round18 accepted frontend improvement",
                "representation_family": cand,
                "sharing_strategy": "official evaluate.py candidate from round18 shard",
                "source": RUN_ID,
                "confidence": "high",
                "novelty": "exploit",
...
namespace["read_rows"] = read_rows_round18
namespace["update_search_state"] = update_search_state_round18
namespace["update_hypothesis_bank"] = update_hypothesis_bank_round18
namespace["main"]()
